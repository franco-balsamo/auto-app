...
type Props = NativeStackScreenProps<HomeStackParamList, 'EditReminder'>;

export default function EditReminderScreen({ route, navigation }: Props) {
  const { vehicleId, reminderId } = route.params;
  const { reminders, updateReminder, deleteReminder } = useReminders(vehicleId);
  ...
  useEffect(() => {
    ...
    setDueKm(reminder.due_km ? String(reminder.due_km) : '');
  }, [reminder]);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const { error } = await updateReminder(reminderId, {
      title: title.trim(),
  ...
  function handleDelete() {
    Alert.alert('Borrar recordatorio', 'No se puede deshacer.', [
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Pressable style={[styles.deleteBtn, deleting && styles.saveBtnDisabled]} onPress={handleDelete} disabled={deleting}>
        <Text style={styles.deleteBtnText}>{deleting ? 'Borrando...' : 'Borrar recordatorio'}</Text>
      </Pressable>
    </ScrollView>
  );
}
